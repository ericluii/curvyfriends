...
if ($graphMethods->getCommentGraphData() === null) {
	...
	$commentData = $graphMethods->createCommentGraphData($friendsToMe, $meToFriends);
}
else {
	$commentData = $graphMethods->getCommentGraphData();
}

if(count($commentData) == 0) {
	array_unshift($commentData, array(gmdate("M d, Y", time()), 0, 0 ,0));
}

array_unshift($commentData, array("Week", "Overall Comments", $basicInfo['friends']['data'][0]['first_name'] . " on " . $basicInfo['first_name'] . "'s wall", $basicInfo['first_name'] . " on " . $basicInfo['friends']['data'][0]['first_name'] . "'s wall"));
