...
if ($graphMethods->getPostGraphData() === null) {
	...
	$postData = $graphMethods->createPostGraphData($friendsToMe, $meToFriends);
}
else {
	$postData = $graphMethods->getCommentGraphData();
}

if(count($postData) == 0) {
	array_unshift($postData, array(gmdate("M d, Y", time()), 0, 0 ,0));
}

array_unshift($postData, array("Week", "Overall Posts", $basicInfo['friends']['data'][0]['first_name'] . " to " . $basicInfo['first_name'], $basicInfo['first_name'] . " to " . $basicInfo['friends']['data'][0]['first_name']));
